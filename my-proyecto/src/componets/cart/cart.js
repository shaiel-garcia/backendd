        ...
        <section className="py-4 container">
            <div className="row justify-content-center">
                <div className="col-12">
                    <h5>Carrito({totalUniqueItems}) Total: ({totalItems})</h5>
                            ...
                            {items.map((item, index) => {
                                return (
                                    <tr key={index}>
                                        ...
                                        <td>{item.title}</td>
                                        <td>{item.price}</td>
                                        <td>Cantidad del producto ({item.quantity})</td>
                                        <td><button className="btn btn-info ms-2" onClick={() => updateItemQuantity(item.id, item.quantity - 1)}>-</button></td>
                                        <td><button className="btn btn-info ms-2" onClick={() => updateItemQuantity(item.id, item.quantity + 1)}>+</button></td>
                                        <td><button className="btn btn-danger ms-2" onClick={() => removeItem(item.id)} >Remover articulo</button></td>
                                    </tr>)
                            })}
                        </tbody>
                    </table>
                ...
                <div className="col-auto">
                    ...
                    <button className="btn btn-primary m-2">Nueva Compra</button>
                </div>
            </div>
        </section>
    );
